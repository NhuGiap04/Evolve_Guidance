...
from seg.diffusers_patch.pipeline_using_Stein_SD import pipeline_using_stein_sd
...
def parse_args():
    ...
    parser.add_argument("--steer-start", type=int, default=None, help="Steering start inference-step index (0-based).")
    parser.add_argument("--steer-end", type=int, default=None, help="Steering end inference-step index (0-based).")
    parser.add_argument(
...
def main():
    ...
    with torch.no_grad():
        result = pipeline_using_stein_sd(pipe, **call_kwargs)
        ...
        for step_idx, step_latents_cpu in enumerate(all_latents, start=1):
            for offset in range(0, step_latents_cpu.shape[0], args.trace_decode_batch_size):
                ...
                with torch.inference_mode():
                    step_images = decode_latents_sd(pipe, step_chunk.to(device=device, dtype=inference_dtype))
