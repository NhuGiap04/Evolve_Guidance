...
def path_to_exp_name(fpath):
    parts = Path(fpath).parts
    if 'sample_evaluation' in parts:
        sample_eval_idx = parts.index('sample_evaluation')
        if sample_eval_idx > 0:
            return parts[sample_eval_idx - 1]
...
path = '../outputs/*/*/*/*/sample_evaluation/*/*_eval.txt'

paths = sorted(glob.glob(path))
if not paths:
    raise SystemExit(f'No eval files matched: {path}')

exp_names = [path_to_exp_name(x) for x in paths]
