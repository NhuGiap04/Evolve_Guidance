...
def file_to_exp_info(file):
    """Return a generic experiment label for the converted eval file."""
    parent_dir = Path(file).parent
    info_path = parent_dir / 'info.json'

    if info_path.exists():
        with info_path.open() as f:
            info = json.load(f)

        for key in ('experiment_name', 'run_name', 'method', 'name'):
            value = info.get(key)
        ...
        if scalar_items:
            return _slugify('_'.join(f'{key}-{value}' for key, value in scalar_items))

    return _slugify(parent_dir.name or 'samples')
...
@click.option(
    '--glob_expression',
    default="../outputs/*/*/*/*/sample_evaluation/*/text_samples.jsonl",
