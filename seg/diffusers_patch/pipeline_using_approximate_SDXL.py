...
def pipeline_using_approximate_sdxl(
    ...
    **kwargs,
    ...
    """Run SDXL denoising with approximate Stein guidance."""
    ...
    if x0_anchor_model not in {"base", "dpm", "lcm", "dmd2"}:
        raise ValueError("x0_anchor_model must be one of {'base', 'dpm', 'lcm', 'dmd2'}")
    if x0_anchor_model in {"lcm", "dmd2"} and x0_anchor_lora_path is None:
        raise ValueError("x0_anchor_lora_path must be set when x0_anchor_model is 'lcm' or 'dmd2'.")

    x0_anchor_adapter_name = "x0_anchor"
    x0_anchor_lora_loaded = False
    x0_anchor_scheduler = None
    if x0_anchor_model in {"dpm", "lcm", "dmd2"}:
    ...
    def _maybe_use_x0_anchor_lora():
        if x0_anchor_model not in {"lcm", "dmd2"} or x0_anchor_lora_path is None:
            yield
            return
...
pipeline_using_stein_sdxl = pipeline_using_approximate_sdxl

__all__ = ["pipeline_using_approximate_sdxl", "pipeline_using_stein_sdxl"]
