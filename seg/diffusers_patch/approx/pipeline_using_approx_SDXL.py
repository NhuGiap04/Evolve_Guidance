...
def pipeline_using_approx_sdxl(
    ...
    **kwargs,
    ...
    def _compute_soft_good_score(
        ...
        t_int_local = _to_timestep_int(t)
        pred_x0_chunks: List[torch.Tensor] = []
        ...
        for start_idx in range(0, current_latents.shape[0], reward_chunk_size):
            ...
            if not torch.is_tensor(reward_chunk):
                reward_chunk = torch.tensor(reward_chunk, device=current_latents.device, dtype=current_latents.dtype)
            reward_chunk = reward_chunk.to(device=current_latents.device, dtype=current_latents.dtype).flatten()
            pred_x0_chunks.append(pred_x0_chunk.detach())
            reward_chunks.append(reward_chunk.detach())

        predicted_x0 = torch.cat(pred_x0_chunks, dim=0)
        rewards = torch.cat(reward_chunks, dim=0)

        alpha_bar_t = self.scheduler.alphas_cumprod[t_int_local].to(
        ...
        score_grouped = torch.zeros_like(latents_grouped)

        for group_idx in range(base_sample_count):
            x = latents_grouped[group_idx].reshape(num_particles, -1)
            z = predicted_grouped[group_idx].reshape(num_particles, -1)
            h_reward = rewards_grouped[group_idx]
        ...
        score_out = torch.nan_to_num(score_grouped.view(b, c, h, w)).to(current_latents.dtype)
        return (rewards if return_rewards else None), score_out
...
pipeline_using_approx_stein_sdxl = pipeline_using_approx_sdxl
