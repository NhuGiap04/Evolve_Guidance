...
def pipeline_using_stein_sd(
    ...
    **kwargs,
    ...
    """Run Stable Diffusion denoising with optional Stein particle transport guidance."""
    ...
    effective_batch_p = num_particles if batch_p is None else batch_p
    if effective_batch_p < 1:
        raise ValueError("batch_p must be >= 1")
    if num_particles < effective_batch_p:
    ...
    steer_start_effective = 0 if steer_start is None else int(steer_start)
    steer_end_effective = total_inference_steps - 1 if steer_end is None else int(steer_end)
    ...
    if steer_start_effective > steer_end_effective:
        raise ValueError("steer_start must be <= steer_end when using inference-step indexing (0-based).")

    use_stein = reward_fn is not None and stein_loop > 0 and stein_step > 0
    reward_chunk_size = max(1, int(effective_batch_p) * base_sample_count)
    ...
    def _slice_condition_tensor(
        ...
        if self.do_classifier_free_guidance:
            ...
            if condition.shape[0] == expected_cfg_batch:
                ...
                return torch.cat([condition_uncond[start_idx:end_idx], condition_text[start_idx:end_idx]], dim=0)
            raise ValueError(
    ...
    def _predict_noise(
        ...
        if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
            added_cond_kwargs = {
                "image_embeds": [_slice_condition_tensor(embed, start_idx, end_idx) for embed in image_embeds]
            }
    ...
    if output_type != "latent" and self.safety_checker is not None:
    ...
    if intermediate_rewards:
        return {"images": image, "nsfw_content_detected": has_nsfw_concept, "intermediate_rewards": intermediate_rewards_data}
    return output
