#!/usr/bin/env python3
"""Batch runner for runs/single/gradient_sd_dpm.py using prompts from .txt or .json."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run gradient_sd_dpm.py over prompts from a .txt or .json file.",
    )
    ...
    parser.add_argument(
        ...
        dest="sd_script",
        type=Path,
        default=Path("runs/single/gradient_sd_dpm.py"),
        help="Path to the single-prompt gradient SD DPM script.",
    )
    parser.add_argument(
        "--python",
        type=str,
...
def _extract_reward_stats(stdout: str) -> Dict[str, str]:
    """Parse final steering/eval reward stats from gradient_sd_dpm.py stdout."""
    # Looks for:
...
def main() -> int:
    ...
    if not args.sd_script.exists():
        print(_c(f"Gradient SD DPM script not found: {args.sd_script}", _Style.RED, _Style.BOLD))
        return 2
    ...
    _title("Gradient SD DPM Batch Runner")
