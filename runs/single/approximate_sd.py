...
from seg.diffusers_patch.pipeline_using_approximate_SD import pipeline_using_approximate_sd
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Approximate SD Stein-guided sampling with per-step reward traces and plots."
    )
    parser.add_argument(
        "--config",
        type=str,
...
def _save_intermediate_step_images(
    ...
    for step_idx, latents_cpu in enumerate(step_latents_cpu_list):
        ...
        with torch.no_grad():
            ...
            images = decode_latents_sd(pipe, latents_cpu.to(device=device, dtype=inference_dtype))
...
def main():
    ...
    with torch.no_grad():
        result = pipeline_using_approximate_sd(pipe, **call_kwargs)
