#!/usr/bin/env python3
"""Batch runner for runs/single/gradient_sdxl_dpm.py using prompts from .txt or .json."""
...
def _run_prompt_shard(
    ...
    for local_idx, (global_idx, prompt) in enumerate(shard_prompts, start=1):
        ...
        cmd = _build_sdxl_cmd(args, prompt, run_output_dir, device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run gradient_sdxl_dpm.py over prompts from a .txt or .json file.",
    )
    ...
    parser.add_argument(
        ...
        dest="sdxl_script",
        type=Path,
        default=Path("runs/single/gradient_sdxl_dpm.py"),
        help="Path to the single-prompt gradient SDXL DPM script.",
    )
    parser.add_argument(
        "--python",
        type=str,
        default=sys.executable,
        help="Python executable used to launch gradient_sdxl_dpm.py.",
    )
    parser.add_argument("--config", type=str, default="pick", choices=["pick", "clip", "seg"])
    parser.add_argument("--negative-prompt", type=str, default="")
    parser.add_argument("--output-dir", type=Path, default=Path("logs/sdxl_batch_dpm"))
    parser.add_argument(
        "--eval-reward",
        type=str,
...
def _extract_reward_stats(stdout: str) -> Dict[str, str]:
    """Parse final steering/eval reward stats from gradient_sdxl_dpm.py stdout."""
    # Looks for:
...
def main() -> int:
    ...
    if not args.sdxl_script.is_absolute() and not args.sdxl_script.exists():
        args.sdxl_script = (repo_root / args.sdxl_script).resolve()
    ...
    if not args.sdxl_script.exists():
        print(_c(f"Gradient SDXL DPM script not found: {args.sdxl_script}", _Style.RED, _Style.BOLD))
        return 2
    ...
    _title("Gradient SDXL DPM Batch Runner")
