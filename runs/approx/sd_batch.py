#!/usr/bin/env python3
"""Batch runner for runs/single/approx_sd.py using prompts from .txt or .json."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run runs/single/approx_sd.py over prompts from a .txt or .json file.",
    )
    ...
    parser.add_argument(
        ...
        dest="sd_script",
        type=Path,
        default=Path("runs/single/approx_sd.py"),
        help="Path to the single-prompt approx SD script.",
    )
    parser.add_argument(
        "--python",
        type=str,
        default=sys.executable,
        help="Python executable used to launch runs/single/approx_sd.py.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("logs/sd_approx_batch"))
...
def main() -> int:
    ...
    if not args.sd_script.exists():
        print(_c(f"Approx SD script not found: {args.sd_script}", _Style.RED, _Style.BOLD))
        return 2
